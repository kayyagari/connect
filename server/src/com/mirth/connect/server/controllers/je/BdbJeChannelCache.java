...
public class BdbJeChannelCache extends DefaultChannelController.ChannelCache {
    private Database db;
    private Environment env;
    private ObjectXMLSerializer objectSerializer;

    private static Logger logger = Logger.getLogger(BdbJeChannelCache.class);

    public BdbJeChannelCache(Environment env, Database db) {
        super();
        this.db = db;
        this.env = env;
        ...
        try {
            ...
            while(cursor.getNext(key, data, null) == OperationStatus.SUCCESS) {
                ...
                if (!cacheById.containsKey(id) || revision > cacheById.get(id).getRevision()) {
                    ...
                    Channel oldItem = cacheById.get(id);
                    Channel item = objectSerializer.deserialize(cr.getChannel().toString(), Channel.class);
            ...
            for (String id : cacheById.keySet()) {
                if (!databaseRevisions.containsKey(id)) {
                    // Remove from cache
                    Channel item = cacheById.remove(id);
                    if (isNameUnique()) {
